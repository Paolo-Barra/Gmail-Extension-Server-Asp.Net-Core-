...
using CommonDesk.Venue.Authentication.TwoFactor.Google;
using CommonDesk.Venue.Authorization;
using CommonDesk.Venue.Authorization.Accounts.Dto;
using CommonDesk.Venue.Authorization.Users;
using CommonDesk.Venue.MultiTenancy;
using CommonDesk.Venue.Web.Authentication.JwtBearer;
using CommonDesk.Venue.Web.Authentication.TwoFactor;
using CommonDesk.Venue.Web.Models.TokenAuth;
using CommonDesk.Venue.Authorization.Impersonation;
using CommonDesk.Venue.Identity;
using CommonDesk.Venue.Net.Sms;
using CommonDesk.Venue.Notifications;
using CommonDesk.Venue.Web.Authentication.External;
using CommonDesk.Venue.Authorization.Users.Dto;

namespace CommonDesk.Venue.Web.Controllers
{
    [Route("api/[controller]/[action]")]
    public class TokenAuthController : VenueControllerBase
    {
        ...
        public async Task LogOut()
        {
            try
            {
                Console.WriteLine("TokehAuthController:Logout");
                if (AbpSession.UserId != null)
                {
                    var tokenValidityKeyInClaims = User.Claims.First(c => c.Type == AppConsts.TokenValidityKey);

                    Console.WriteLine("TokehAuthController:GetUser");
                    var user = _userManager.GetUser(AbpSession.ToUserIdentifier());
                    if(user != null)
                    {
                        Console.WriteLine($"User=[{user}]");
                        Console.WriteLine("TokehAuthController:RemoveTokenValidityKeyAsync");
                    ...
                    {
                        Console.WriteLine("TokehAuthController:Logout:User not found");
                    }                    
                }
            }
            catch(Exception ex) 
            {
                Console.WriteLine($"TokehAuthController:Error in Logout:[{ex.Message}]:Stack=[{ex.StackTrace}]");
            }
        }
                    ...
                    case AbpLoginResultType.Success:
                        {
                            ...
                            return new ExternalAuthenticateResultModel
                            {
                                ...
                                //COMMONDESK
                                UserId = loginResult.User.Id,
                                RefindUserAccountId = loginResult.User.UserAccountId
                            };
                        }
        ...
        // COMMONDESK: Return a user by its user account id
        [HttpGet]
        public bool IsUserRegistered(GetUserByUserAccountIdInput input) 
        {
            var user = _userManager.GetUserByUserAccountId(input.ProviderId, input.UserAccountId);
            return user != null;
        }  
    }
}
