...
namespace CommonDesk.Venue.Web.Authentication.JwtBearer
{
    public class VenueJwtSecurityTokenHandler : ISecurityTokenValidator
    {
        private readonly JwtSecurityTokenHandler _tokenHandler;

        public VenueJwtSecurityTokenHandler()
        {
                ...
                using (var uow = unitOfWorkManager.Object.Begin())
                {
                    ...
                    using (unitOfWorkManager.Object.Current.SetTenantId(userIdentifier.TenantId))
                    {
                        using (var userManager = IocManager.Instance.ResolveAsDisposable<UserManager>())
                        {
                            ...
                            if(user == null)
                            {
                                Console.WriteLine($"ValidateToken:User not found");
                                uow.Complete();
                                return principal;
                            }
