...
namespace CommonDesk.Venue.Authorization.Users
{
    ...
    public class UserManager : AbpUserManager<Role, User>
    {
        ...
        public async Task<User> GetUserAsync(UserIdentifier userIdentifier)
        {
            ...
            if (user == null)
            {
                Console.WriteLine("GetUserAsync:There is no user: " + userIdentifier);
            }
        ...
        {
            return _localizationManager.GetString(VenueConsts.LocalizationSourceName, name);
        }

        // COMMONDES: Find if user exists by the user account id
        public User GetUserByUserAccountId(string providerId, string userAccountId) 
        {
            try {
                var user = AsyncHelper.RunSync(() => FindByLoginAsync(providerId, userAccountId));
                return user;
            }
            catch(Exception e)
            {
                Console.WriteLine($"UM:DoesUserAccountExists:Error=[{e.Message}]");
            }
